...
object WynnOverhaulToastQueue {
    ...
    fun current(): Toast? {
        ...
        val settings = WynnOverhaulConfig.current.toast(next.kind)
    ...
    private fun durationFor(toast: Toast): Long =
        (WynnOverhaulConfig.current.toast(toast.kind).durationSeconds.coerceIn(1.5, 20.0) * 1000).toLong()
}
