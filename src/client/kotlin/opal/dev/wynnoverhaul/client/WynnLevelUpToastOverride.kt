...
object WynnLevelUpToastOverride {
    ...
    private fun petToast(lines: List<String>) {
        ...
        WynnOverhaulToastQueue.show(WynnOverhaulToastQueue.make(WynnOverhaulToastQueue.Kind.LEVEL_UP, "Pet Level Up!", subtitle, PET_TOAST_COLOR, progress))
    }

    private fun onMessage(message: Component): Boolean {
        if (!WynnOverhaulGate.inGame) return true
        if (!WynnOverhaulConfig.current.levelUpToastEnabled) return true
        ...
        PERSONAL_GENERIC.matchEntire(text)?.let { m ->
            ...
            if (name == Minecraft.getInstance().player?.name?.string) {
                ...
                WynnOverhaulToastQueue.show(WynnOverhaulToastQueue.make(WynnOverhaulToastQueue.Kind.LEVEL_UP, name, "reached $level", OTHER_TOAST_COLOR))
            }
            return false
        }
