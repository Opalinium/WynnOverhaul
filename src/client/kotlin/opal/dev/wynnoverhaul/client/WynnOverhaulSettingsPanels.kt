...
class WynnOverhaulSettingsPanels(private val host: Host) {
    interface Host {
        val screen: Screen
        val panelFont: Font
        fun rebuildPanels()
    ...
    private val config = WynnOverhaulConfig.current
    ...
    private var activeToastKind: WynnOverhaulToastQueue.Kind = WynnOverhaulToastQueue.Kind.QUEST
    ...
    fun buildTabs(left: Int, w: Int, top: Int): Int {
        ...
        if (activeGroup == Group.NOTIFICATIONS) {
            ...
            if (activeNotificationTab == NotificationTab.TOASTS) {
                cy = drawTabRow(WynnOverhaulToastQueue.Kind.entries, contentLeft, contentW, cy, { it.label }, { it == activeToastKind }, sub = true) {
                    activeToastKind = it
                    host.rebuildPanels()
                }
            }
    ...
    private fun buildRulesTab(left: Int, w: Int, top: Int) {
        ...
        host.addPanelWidget(OwButton(left, y, 96, 18, Component.literal("Add rule")) { addRule(WynnOverhaulConfig.TrackerRule()) })
        val chestButton = OwButton(left + 100, y, 148, 18, Component.literal("Add loot chest rule")) {
            addRule(WynnOverhaulConfig.TrackerRule(nameMode = "CONTAINS", namePattern = "Loot Chest", colorArgb = 0xFFFFD700L, trackBlockBelow = true))
        }
        ...
        val nodeButton = OwButton(left + 252, y, 166, 18, Component.literal("Add gathering node rule")) {
            addRule(WynnOverhaulConfig.TrackerRule(nameMode = "CONTAINS", namePattern = "Mining", colorArgb = 0xFF55FF55L, trackGatheringNode = true, nodeProfession = "MINING"))
        }
    ...
    private fun addRuleRows(rule: WynnOverhaulConfig.TrackerRule, index: Int, top: Int, left: Int, rowWidth: Int) {
    ...
    private fun addRule(rule: WynnOverhaulConfig.TrackerRule) {
    ...
    private fun tierLabel(rule: WynnOverhaulConfig.TrackerRule): String = if (rule.minChestTier <= 0) "any tier" else "T${rule.minChestTier}+"

    private fun professionLabel(rule: WynnOverhaulConfig.TrackerRule): String =
        if (rule.nodeProfession.isBlank()) "any profession" else rule.nodeProfession.lowercase().replaceFirstChar { it.uppercase() }

    private fun targetLabel(rule: WynnOverhaulConfig.TrackerRule): String = when {
    ...
    private fun cycleTarget(rule: WynnOverhaulConfig.TrackerRule) {
        when {
    ...
    private fun buildAnimationsTab(left: Int, w: Int, top: Int) {
        panel(left, w, top) {
            ...
            cycleButton({ "Weapon animation models..." }, "Register weapon models and choose which animation each one uses.") {
                Minecraft.getInstance().setScreenAndShow(WynnOverhaulWeaponAnimationScreen(host.screen))
            }
            checkbox(
    ...
    private fun buildQolTab(left: Int, w: Int, top: Int) {
        panel(left, w, top) {
            ...
            checkbox(
                ...
                "Shows the tooltip of your currently equipped item beside the hovered armor piece, accessory or weapon in the WynnOverhaul inventory.",
            ...
            rows += OwButton(left, 0, w, OwTheme.ROW_H - 2, Component.literal("Quest Reference (wiki)")) {
                Minecraft.getInstance().setScreenAndShow(WynnOverhaulQuestBookScreen(host.screen))
            } to OwTheme.ROW_H
        }
    }
    ...
    private fun buildToastsTab(left: Int, w: Int, top: Int) {
        panel(left, w, top) {
            ...
            val (enabledLabel, enabledTooltip) = when (kind) {
                WynnOverhaulToastQueue.Kind.QUEST -> "Quest completion toast" to "Blocks the \"[Quest Completed]\" style chat messages (quests, mini-quests, caves, dungeons, raids, world events, boss altars) and shows a HUD toast with the rewards instead."
                WynnOverhaulToastQueue.Kind.LEVEL_UP -> "Level up toast" to "Blocks level-up chat messages and shows a HUD toast instead."
                WynnOverhaulToastQueue.Kind.DISCOVERY -> "Area discovery toast" to "Blocks the \"Area Discovered\" chat message and its description, and shows a HUD toast instead."
                WynnOverhaulToastQueue.Kind.LOCATION -> "Location change toast" to "Shows a HUD toast with the region name when you enter a new area."
            }
            ...
            button("Preview this toast", "Shows a sample of this toast using your current settings.") { WynnOverhaulToastQueue.preview(kind) }
            button("Preview all toasts", "Queues one sample of every toast type.") { WynnOverhaulToastQueue.preview() }
    ...
    private fun toastEnabled(kind: WynnOverhaulToastQueue.Kind): Boolean = when (kind) {
        WynnOverhaulToastQueue.Kind.QUEST -> config.questCompletionToastEnabled
        WynnOverhaulToastQueue.Kind.LEVEL_UP -> config.levelUpToastEnabled
        WynnOverhaulToastQueue.Kind.DISCOVERY -> config.discoveryToastEnabled
        WynnOverhaulToastQueue.Kind.LOCATION -> config.locationToastEnabled
    }

    private fun setToastEnabled(kind: WynnOverhaulToastQueue.Kind, value: Boolean) {
        when (kind) {
            WynnOverhaulToastQueue.Kind.QUEST -> config.questCompletionToastEnabled = value
            WynnOverhaulToastQueue.Kind.LEVEL_UP -> config.levelUpToastEnabled = value
            WynnOverhaulToastQueue.Kind.DISCOVERY -> config.discoveryToastEnabled = value
            WynnOverhaulToastQueue.Kind.LOCATION -> config.locationToastEnabled = value
        }
    }

    private fun setToastStyle(kind: WynnOverhaulToastQueue.Kind, value: String) {
        when (kind) {
            WynnOverhaulToastQueue.Kind.QUEST -> config.questToastStyle = value
            WynnOverhaulToastQueue.Kind.LEVEL_UP -> config.levelUpToastStyle = value
            WynnOverhaulToastQueue.Kind.DISCOVERY -> config.discoveryToastStyle = value
            WynnOverhaulToastQueue.Kind.LOCATION -> config.locationToastStyle = value
        }
    }
    ...
    private fun buildOverridesTab(left: Int, w: Int, top: Int) {
        panel(left, w, top) {
            ...
            contentBookCheckbox.setTooltip(Tooltip.create(Component.literal("Right-clicking the Content Book item opens WynnOverhaul's own menu instead of Wynncraft's.")))
            ...
            waypointCheckbox.setTooltip(
                Tooltip.create(
                    Component.literal(
                        "Renders WynnOverhaul's own waypoint pointer at Wynncraft's real quest marker location, " +
            ...
            debugCheckbox.setTooltip(
                Tooltip.create(
                    Component.literal(
                        "Middle-click a hovered item in a custom WynnOverhaul screen to copy its registry id, name and full data components to the clipboard -- for matching server menu items during development.",
                    ),
                ),
            )
