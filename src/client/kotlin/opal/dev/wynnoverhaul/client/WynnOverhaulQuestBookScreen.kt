...
class WynnOverhaulQuestBookScreen(parent: Screen) : OwScreen(Component.literal("WynnOverhaul - Quest Book"), parent) {
        ...
        if (trackedQuest != null) {
            addRenderableWidget(
                OwButton(left, y, w, OwTheme.ROW_H - 2, Component.literal("Tracked: ${trackedQuest.name}"), accent = true) {
                    Minecraft.getInstance().setScreenAndShow(WynnOverhaulQuestDetailScreen(trackedQuest, this))
                },
            )
            y += OwTheme.ROW_H
        }
