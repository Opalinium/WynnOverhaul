...
object TrackerNodeStore {
    private val PATH = FabricLoader.getInstance().configDir.resolve("wynnoverhaul-nodes.json")
    ...
    private fun write(nodes: Map<Long, NodeRecord>) {
        ...
        try {
            ...
            if (!loggedSaveError) {
                ...
                WynnOverhaul.LOGGER.error("Failed to save tracked gathering nodes", t)
            }
        }
    }
    ...
    private fun readAll(): MutableMap<String, MutableMap<String, NodeRecord>> {
        return try {
            ...
            if (!loggedLoadError) {
                ...
                WynnOverhaul.LOGGER.error("wynnoverhaul-nodes.json unreadable, starting a fresh store", t)
            }
            mutableMapOf()
        }
    }
