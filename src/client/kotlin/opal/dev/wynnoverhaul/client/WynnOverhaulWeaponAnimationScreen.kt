...
class WynnOverhaulWeaponAnimationScreen(parent: Screen? = null) : OwScreen(Component.literal("Weapon Animations"), parent) {
    ...
    private fun addHeld() {
        ...
        if (WeaponAnimationRegistry.register(stack) == null) {
            player.sendSystemMessage(Component.literal("[WynnOverhaul] Hold a weapon first.").withStyle(ChatFormatting.RED))
            return
        }
        rebuildWidgets()
    }
