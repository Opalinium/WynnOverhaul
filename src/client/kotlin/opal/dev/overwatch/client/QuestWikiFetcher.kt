...
object QuestWikiFetcher {
    ...
    fun find(type: ActivityType, name: String): WikiPage? {
        ...
        if (fallback == null) {
            ...
            Overwatch.LOGGER.warn(
                "Overwatch activity wiki lookup failed: type={} name='{}' triedKeys=[{}] bundleSize={} sampleKeys={}",
    ...
    private fun load(): Map<String, WikiPage> {
        return try {
            val stream = QuestWikiFetcher::class.java.getResourceAsStream("/assets/overwatch/activity_wiki.json")
            if (stream == null) {
                Overwatch.LOGGER.error("Overwatch activity wiki bundle not found on classpath at /assets/overwatch/activity_wiki.json")
                return emptyMap()
            }
