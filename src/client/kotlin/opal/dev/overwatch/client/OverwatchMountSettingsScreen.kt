...
class OverwatchMountSettingsScreen(private val menu: AbstractContainerMenu) : Screen(Component.literal("Mount Settings")) {
    ...
    override fun init() {
        journalButton = OwButton(0, 0, 0, 0, Component.literal("Journal")) {
            openOverwatchTab(OverwatchInventoryScreen.InvTab.JOURNAL)
        }.also { addRenderableWidget(it) }
        characterButton = OwButton(0, 0, 0, 0, Component.literal("Character")) {
            openOverwatchTab(OverwatchInventoryScreen.InvTab.CHARACTER)
        }.also { addRenderableWidget(it) }
        layoutShortcuts()
    }
    ...
    private fun openOverwatchTab(tab: OverwatchInventoryScreen.InvTab) {
        val client = Minecraft.getInstance()
        val player = client.player ?: return
        try {
            player.closeContainer()
        } catch (t: Throwable) {
            Overwatch.LOGGER.warn("Overwatch mount settings shortcut close threw", t)
        }
        if (player.containerMenu !== player.inventoryMenu) {
            player.containerMenu = player.inventoryMenu
        }
        client.setScreenAndShow(OverwatchInventoryScreen(player.inventoryMenu, tab))
    }
    ...
    override fun mouseClicked(event: MouseButtonEvent, doubleClick: Boolean): Boolean {
        ...
        if (button == 2 && OverwatchItemDebug.tryCopyToClipboard(slot.item)) return true
    ...
    private fun sendMountInput(slot: Int, button: Int, kind: ContainerInput) {
        ...
        try {
            ...
            Overwatch.LOGGER.error("Overwatch mount settings input failed", t)
        }
    }
    ...
    override fun onClose() {
        try {
            ...
            Overwatch.LOGGER.warn("Overwatch mount settings close threw", t)
        }
