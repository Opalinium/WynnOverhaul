...
object WynnQuestCompletionTracker {
    ...
    private fun onMessage(message: Component): Boolean {
        ...
        if (lines.any { it.equals(OBJECTIVE_FINISHED, ignoreCase = true) }) {
            ...
            if (suppress) {
                OverwatchToastQueue.show(
                    OverwatchToastQueue.make(OverwatchToastQueue.Kind.QUEST, OBJECTIVE_FINISHED, "Redeem your prize", TOAST_COLOR, "/guild rewards"),
                )
            }
            return !suppress
        }
        for (line in lines) {
            ...
            if (suppress && !own) {
                OverwatchToastQueue.show(
                    OverwatchToastQueue.make(OverwatchToastQueue.Kind.QUEST, "Guild Objective", "$name finished their weekly objective", TOAST_COLOR),
                )
            }
            return !suppress
        }
    ...
    private fun onQuestCompleted(kind: String, name: String, detail: String, toast: Boolean) {
        ...
        if (toast) {
            OverwatchToastQueue.show(OverwatchToastQueue.make(OverwatchToastQueue.Kind.QUEST, "$kind Completed", name, TOAST_COLOR, detail))
        }
    }
