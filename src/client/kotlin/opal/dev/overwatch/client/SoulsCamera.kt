...
object SoulsCamera {
    ...
    private fun eligible(mc: Minecraft): Boolean {
        if (!OverwatchConfig.current.soulsCameraEnabled) return false
    ...
    @JvmStatic
    fun showsReticle(): Boolean = active && OverwatchConfig.current.soulsCameraReticle
    ...
    fun onMouseTurn(mc: Minecraft, dx: Double, dy: Double): Boolean {
        ...
        val scale = 0.15 * OverwatchConfig.current.soulsCameraSensitivity
    ...
    private fun inCombatHold(mc: Minecraft): Boolean {
        ...
        val holdNanos = (OverwatchConfig.current.soulsCameraFaceHoldMs * 1.0e6).toLong()
    ...
    fun remap(mc: Minecraft, presses: Input, move: Vec2): Move? {
        ...
        val factor = OverwatchConfig.current.soulsCameraTurnSpeed.toFloat()
