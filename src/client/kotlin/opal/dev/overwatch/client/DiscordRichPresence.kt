...
class DiscordRichPresence : ClientModInitializer {
        ...
        try {
            ...
            if (conn == null) {
                if (!loggedConnectError) {
                    ...
                    Overwatch.LOGGER.warn("Overwatch Discord RPC: couldn't find a Discord client to connect to")
                }
                return
            }
            connection = conn
            ...
            if (!loggedConnectError) {
                loggedConnectError = true
                Overwatch.LOGGER.warn("Overwatch Discord RPC: couldn't connect ({})", t.message)
            }
        }
    }
    ...
    private fun updatePresence(mc: Minecraft) {
        ...
        try {
            ...
            Overwatch.LOGGER.warn("Overwatch Discord RPC: failed to update presence ({})", t.message)
            disconnect()
        }
    }
    ...
    private companion object {
        ...
        const val LARGE_IMAGE_KEY = "overwatch"
